...
<body class="dental-ai-page">
            ...
            <div class="dac">
              ...
              <div class="da-pane" id="daVideoPan">
                ...
                <div class="da-cam-wrap" id="daVidWrap" style="display:none">
                  ...
                  <!-- Recording overlay: countdown + REC badge -->
                  <div id="daVidOverlay" style="display:none;position:absolute;inset:0;background:rgba(2,20,24,.55);flex-direction:column;align-items:center;justify-content:center;gap:10px;pointer-events:none">
                    <div style="display:flex;align-items:center;gap:7px;background:rgba(239,68,68,.92);padding:5px 14px;border-radius:999px">
                      <span style="width:9px;height:9px;border-radius:50%;background:#fff;display:inline-block;animation:daRecBlink 1s ease-in-out infinite"></span>
                      <span style="color:#fff;font-size:12px;font-weight:700;letter-spacing:.04em">REC</span>
                    </div>
                    <div id="daVidCdNum" style="font-size:60px;font-weight:900;color:#fff;line-height:1;text-shadow:0 2px 14px rgba(0,0,0,.6)">10</div>
                    <div style="color:rgba(255,255,255,.75);font-size:12px">giây còn lại</div>
                  </div>
                ...
                <!-- Progress bar (recording + extracting) -->
                <div id="daVidProgress" style="display:none;margin-bottom:10px">
                  <div style="display:flex;justify-content:space-between;align-items:center;margin-bottom:5px">
                    <span style="font-size:12px;font-weight:600" id="daVidProgressText">Đang ghi video...</span>
                    <span style="font-size:12px;color:#64748b" id="daVidProgressSec">10s</span>
                  </div>
                  <div style="height:5px;background:#e0f0f4;border-radius:3px;overflow:hidden">
                    <div id="daVidProgressBar" style="height:100%;width:0;border-radius:3px;transition:width .9s linear"></div>
                  </div>
                </div>
                <div class="da-cam-bar" id="daVidBar" style="display:none">
                  <button type="button" class="btn-capture" id="daVidRecBtn">
                    <i class="bi bi-record-circle-fill"></i> Bắt đầu quay 10s
                  </button>
                ...
                <button type="button" class="btn btn-outline-secondary w-100" id="daStartVidBtn"
                  style="border-radius:10px;font-size:13px;font-weight:600;padding:10px">
                  <i class="bi bi-camera-video-fill me-2"></i>Bật Camera để quay video
                </button>
              </div>
  ...
  <script>
    (function() {
      ...
      /* ── Video — quay 10 giây rồi trích frame ── */
      const VID_SECS = 10;
      ...
      $('daVidRecBtn').addEventListener('click', startRecording);
      ...
      async function startVid() {
        ...
        if (!secure || !navigator.mediaDevices?.getUserMedia) {
          showVidErr('Camera yêu cầu HTTPS hoặc localhost.');
          return;
        }
        try {
          ...
          const msgs = {
            NotAllowedError:  'Trình duyệt đang chặn camera — cấp quyền trong thanh địa chỉ',
            NotFoundError:    'Không tìm thấy camera',
            NotReadableError: 'Camera đang dùng bởi app khác',
          };
          showVidErr(msgs[err.name] || 'Không thể mở camera: ' + (err.message || err.name));
        }
      }

      function showVidErr(msg) {
        $('daVidErrMsg').textContent = msg;
        $('daVidErr').style.display  = 'flex';
      }

      function startRecording() {
        if (!vidStream || (mediaRecorder && mediaRecorder.state === 'recording')) return;

        recordedChunks = [];
        const mimeType = ['video/webm;codecs=vp9','video/webm;codecs=vp8','video/webm','video/mp4']
          .find(t => MediaRecorder.isTypeSupported(t)) || '';
        mediaRecorder = new MediaRecorder(vidStream, mimeType ? { mimeType } : {});
        mediaRecorder.ondataavailable = e => { if (e.data.size > 0) recordedChunks.push(e.data); };
        mediaRecorder.onstop = processVideo;
        mediaRecorder.start(200);

        let remaining = VID_SECS;
        const btn = $('daVidRecBtn');
        btn.disabled     = true;
        btn.innerHTML    = '<i class="bi bi-record-circle-fill" style="color:#ef4444"></i> Đang ghi...';

        const overlay  = $('daVidOverlay');
        const progress = $('daVidProgress');
        const bar      = $('daVidProgressBar');
        ...
        bar.style.width        = '0%';
        $('daVidCdNum').textContent        = remaining;
        $('daVidProgressText').textContent = 'Đang ghi video...';
        $('daVidProgressText').style.color = '#ef4444';
        $('daVidProgressSec').textContent  = remaining + 's';

        cdTimer = setInterval(() => {
          remaining--;
          $('daVidCdNum').textContent       = remaining;
          $('daVidProgressSec').textContent = remaining + 's';
          bar.style.width = ((VID_SECS - remaining) / VID_SECS * 100) + '%';
          if (remaining <= 0) {
      ...
      async function processVideo() {
        $('daVidOverlay').style.display  = 'none';
        const bar = $('daVidProgressBar');
        bar.style.background  = 'linear-gradient(90deg,#0f766e,#049ebb)';
        bar.style.transition  = 'width .4s ease';
        bar.style.width       = '0%';
        $('daVidProgressText').textContent = 'Đang trích xuất ảnh từ video...';
        $('daVidProgressText').style.color = '#049ebb';
        $('daVidProgressSec').textContent  = '';

        const blob  = new Blob(recordedChunks, { type: mediaRecorder.mimeType || 'video/webm' });
        const url   = URL.createObjectURL(blob);
        const snapAt = [2, 4, 6, 8];

        for (let i = 0; i < snapAt.length; i++) {
          const frame = await extractFrame(url, snapAt[i]);
          if (frame) addImage(frame);
          bar.style.width = ((i + 1) / snapAt.length * 100) + '%';
        }
        ...
        const btn  = $('daVidRecBtn');
        btn.disabled  = false;
        btn.innerHTML = '<i class="bi bi-record-circle-fill"></i> Quay thêm';
      }

      function extractFrame(videoUrl, t) {
        return new Promise(resolve => {
          const vid = document.createElement('video');
          const cv  = $('daVidCanvas');
          vid.muted       = true;
          ...
          setTimeout(() => { vid.src = ''; resolve(null); }, 6000);
          vid.src = videoUrl;
        });
      }
