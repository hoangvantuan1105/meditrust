<div class="card mb-3">
    <div class="card-header d-flex justify-content-between align-items-center">
        <strong>Cập nhật vật tư sử dụng</strong>
        ...
        <table class="table table-bordered">
            ...
            <tbody>
                <?php foreach ($vatTuList as $vt):
                    $checked = isset($vatTuDaChon[$vt['id']]);
                    $soLuongCu = $checked ? $vatTuDaChon[$vt['id']]['so_luong'] : '';
                ?>
                    <tr>
                        <td>
                            <input type="checkbox" class="vt-check" data-gia="<?= $vt['gia_nhap'] ?>" <?= $checked ? 'checked' : '' ?>>
                        ...
                        <td>
                            <input type="number" name="so_luong[]" class="form-control so-luong" min="1"
                                value="<?= $soLuongCu ?>" <?= $checked ? '' : 'disabled' ?>>
                        </td>
                        <td class="thanh-tien">
                            <?= $checked ? number_format($vt['gia_nhap'] * $soLuongCu) . ' đ' : '0 đ' ?>
                        </td>
                    </tr>
                <?php endforeach; ?>
            </tbody>
    ...
    function confirmSelection() {
        ...
        try {
            if (window.opener && !window.opener.closed && typeof window.opener.receiveMaterials === 'function') {
                ...
                alert("Không tìm thấy cửa sổ cha để gửi dữ liệu! Hãy đảm bảo bạn mở trang này từ nút 'Sửa vật tư'.");
            }
        } catch (e) {
            console.error(e);
            alert("Có lỗi xảy ra khi gửi dữ liệu: " + e.message);
            window.close(); // Đóng cửa sổ ngay cả khi lỗi để tránh treo
        }
    }
