            ...
            <div class="card shadow mb-4">
                ...
                <div class="card-body">
                    ...
                    <ul class="nav nav-tabs mb-3" id="examTabs">
                        <li class="nav-item">
                            <a class="nav-link active" data-toggle="tab" href="#tabKham">
                                <i class="fas fa-stethoscope mr-1"></i> Khám & Thuốc
                            </a>
                        </li>
                        <li class="nav-item">
                            <a class="nav-link" data-toggle="tab" href="#tabXray" id="xrayTabLink">
                                <i class="fas fa-x-ray mr-1"></i> X-Quang
                                <span class="badge badge-primary ml-1" id="xrayCount">0</span>
                            </a>
                        </li>
                    </ul>
                    ...
                    <div class="tab-content">
                        <!-- ===== TAB KHÁM ===== -->
                        <div class="tab-pane fade show active" id="tabKham">
                        ...
                        <!-- ===== TAB X-QUANG ===== -->
                        <div class="tab-pane fade" id="tabXray">
                            <div class="d-flex justify-content-between align-items-center mb-3">
                                <h6 class="text-primary mb-0"><i class="fas fa-x-ray mr-1"></i> X-Quang Ca Khám Này</h6>
                                <a href="admin.php?admin=formUploadXray&lich_kham_id=<?= (int)($_GET['id'] ?? 0) ?>"
                                    class="btn btn-primary btn-sm">
                                    <i class="fas fa-upload mr-1"></i> Upload X-Quang
                                </a>
                            </div>

                            <div id="xrayListContainer">
...
<script>
    // Load X-quang khi click tab
    document.getElementById('xrayTabLink').addEventListener('click', function() {
        loadXrayList();
    });

    function loadXrayList() {
        const lichKhamId = <?= (int)($_GET['id'] ?? 0) ?>;
        fetch('admin.php?admin=getXrayByLichKham&lich_kham_id=' + lichKhamId)
            .then(r => r.json())
            .then(list => {
                document.getElementById('xrayCount').textContent = list.length;
                const container = document.getElementById('xrayListContainer');

                if (!list.length) {
                    container.innerHTML = `
                    <div class="text-center py-4 text-muted">
                        <i class="fas fa-x-ray fa-2x mb-2"></i>
                        <p>Chưa có X-quang nào cho ca khám này.</p>
                        <a href="admin.php?admin=formUploadXray&lich_kham_id=${lichKhamId}"
                           class="btn btn-primary btn-sm">
                            <i class="fas fa-upload mr-1"></i> Upload X-Quang Ngay
                        </a>
                    </div>`;
                    return;
                }
                ...
                list.forEach(x => {
                    const loai = loaiMap[x.loai_xray] || x.loai_xray;
                    const ai = x.ai_phan_tich ? '<span class="badge badge-success ml-1"><i class="fas fa-robot"></i> AI</span>' : '';
                    html += `
                <div class="col-6 mb-3">
                    <div class="card border">
                        <div style="height:110px;overflow:hidden;background:#f8f9fc;">
                            <img src="backend/uploads/xray/${x.file_path}"
                                 style="width:100%;height:100%;object-fit:cover;cursor:pointer;"
                                 onclick="window.open(this.src,'_blank')"
                                 onerror="this.parentElement.innerHTML='<div class=d-flex align-items-center justify-content-center h-100 text-muted><i class=fas fa-image></i></div>'">
                        </div>
                        <div class="card-body p-2">
                            <div class="font-weight-bold" style="font-size:12px;">${loai} ${ai}</div>
                            ${x.vi_tri ? `<small class="text-muted">📍 ${x.vi_tri}</small><br>` : ''}
                            <a href="admin.php?admin=viewXray&idAdmin=${x.id}"
                               class="btn btn-xs btn-outline-info mt-1" style="font-size:11px;">
                                Xem chi tiết
                            </a>
                        </div>
                    </div>
            ...
            .catch(() => {
                document.getElementById('xrayListContainer').innerHTML =
                    '<div class="alert alert-warning">Không thể tải danh sách X-quang</div>';
            });
    }
</script>
