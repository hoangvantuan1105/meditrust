            ...
            fetch("admin.php?admin=getAvailableTime", {
                ...
                .then(data => {
                    ...
                    if (!data || data.length === 0) {
                        if (selectedTime) {
                            select.innerHTML += `<option value="${selectedTime}" selected>${selectedTime}</option>`;
                        } else {
                        ...
                        let found = false;
                        data.forEach(time => {
                            let sel = (time === selectedTime) ? "selected" : "";
                            if (sel) found = true;
                            select.innerHTML += `<option value="${time}" ${sel}>${time}</option>`;
                        });
                        if (!found && selectedTime) {
                            select.innerHTML += `<option value="${selectedTime}" selected>${selectedTime} (đã chọn)</option>`;
                        }
                    }
                })
                .catch(() => {
