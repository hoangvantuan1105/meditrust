    ...
    <div class="col-12">
        <div class="card shadow mb-4">
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                <h6 class="m-0 font-weight-bold text-primary">
                    <i class="fas fa-x-ray mr-1"></i> Lịch Sử X-Quang
                    <span class="badge badge-primary ml-1"><?= count($xrayList ?? []) ?></span>
                </h6>
                <?php if (!empty($item['id'])): ?>
                    <a href="admin.php?admin=xrayBenhNhan&idAdmin=<?= htmlspecialchars($item['id']) ?>"
            ...
            <div class="card-body">
                <?php if (empty($xrayList)): ?>
                    <div class="alert alert-info mb-0">Bệnh nhân này chưa có ảnh X-quang nào.</div>
                <?php else: ?>
                    <div class="row">
                        <?php foreach (array_slice($xrayList, 0, 6) as $x):
                            $loaiMap = [
                                'toan_ham' => ['l' => 'Toàn Hàm', 'c' => 'primary'],
                                'rang_cu' => ['l' => 'Răng Cụ Thể', 'c' => 'info'],
                                'cat_loc' => ['l' => 'Cắn Lọc', 'c' => 'secondary'],
                                'cat_ngang' => ['l' => 'Cắt Ngang', 'c' => 'dark'],
                                'cbct_3d' => ['l' => 'CBCT 3D', 'c' => 'warning'],
                            ];
                            $l = $loaiMap[$x['loai_xray']] ?? ['l' => $x['loai_xray'], 'c' => 'secondary'];
                        ?>
                            <div class="col-md-4 col-lg-2 mb-3">
                                <a href="admin.php?admin=viewXray&idAdmin=<?= $x['id'] ?>" class="text-decoration-none">
                                    <div class="card border h-100 text-center" style="cursor:pointer;">
                                        <div style="height:100px;overflow:hidden;background:#f8f9fc;">
                                        ...
                                        <div class="card-body p-1">
                                            <span class="badge badge-<?= $l['c'] ?>" style="font-size:10px;"><?= $l['l'] ?></span>
                                            <div style="font-size:11px;color:#888;"><?= date('d/m/Y', strtotime($x['ngay_chup'])) ?></div>
                                            <?php if (!empty($x['ai_phan_tich'])): ?>
                                                <span class="badge badge-success" style="font-size:9px;"><i class="fas fa-robot"></i></span>
                                            <?php endif; ?>
                                        </div>
                                    </div>
                                </a>
                    ...
                    <?php if (count($xrayList) > 6): ?>
                        <div class="text-center">
                            <a href="admin.php?admin=xrayBenhNhan&idAdmin=<?= htmlspecialchars($item['id'] ?? '') ?>"
                                class="btn btn-sm btn-outline-primary">
                                Xem thêm <?= count($xrayList) - 6 ?> ảnh
                            </a>
                        </div>
                    <?php endif; ?>
                <?php endif; ?>
