...
class modelClinic
{
    ...
    {
        $host = "localhost";
        $db_name = "quan_ly_phong_kham_nha_khoa";
        $username = "root";
        $password = "";
    ...
    public function insertMaterials($data)
    {
        ...
        (ten_vat_tu, don_vi, so_luong, hang_san_xuat, danh_muc, trang_thai, gia_nhap, han_su_dung,trang_thai_han)
        VALUES (?,?,?,?,?,?,?,?,?)";
    ...
    {
        $sql = "SELECT ten_vat_tu, don_vi, gia_nhap, hang_san_xuat, danh_muc 
            FROM vat_tu 
    ...
    public function insertChiTietHoaDon($data)
    {
        $sql = "INSERT INTO chi_tiet_hoa_don 
            (hoa_don_id, loai_item, item_id, so_luong, don_gia, thanh_tien)
            VALUES (?, ?, ?, ?, ?, ?)";
    ...
    public function getGiaDichVuTheoLichKham($lich_kham_id, $dich_vu_id)
    {
        $sql = "
            SELECT ct.don_gia
            FROM ct_ls_kham ct
            JOIN lich_su_kham ls ON ct.ls_kham_id = ls.id
            WHERE ls.lich_kham_id = ? AND ct.dich_vu_id = ?
            LIMIT 1
        ";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([$lich_kham_id, $dich_vu_id]);
        $result = $stmt->fetchColumn();
        return $result !== false ? (float) $result : $this->getGiaDichVu($dich_vu_id);
    }
    ...
    public function autoUpdateDiscountStatus()
    {
        $sql = "UPDATE ma_giam_gia
                SET trang_thai = 0
                WHERE trang_thai = 1
                  AND (ngay_ket_thuc < CURDATE() OR so_luot <= 0)";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute();
    }
    ...
    SELECT
        ...
        lsk.chan_doan,
        lsk.ngay_kham,
        CASE WHEN MIN(hd.id) IS NOT NULL THEN 1 ELSE 0 END AS da_tao_hoa_don,
        MAX(lk.la_tai_kham) AS la_tai_kham
    ...
    LEFT JOIN hoa_don hd
        ON hd.lich_kham_id = lsk.lich_kham_id
    LEFT JOIN lich_kham lk
        ON lk.id = lsk.lich_kham_id
    GROUP BY lsk.id
    ORDER BY lsk.id DESC
    ";
    ...
    public function getChiTietHoaDon($hoa_don_id)
    {
        $sql = "
        SELECT ct.*, 
               COALESCE(dv.ten_dich_vu, t.ten_thuoc) as ten
        FROM chi_tiet_hoa_don ct
        LEFT JOIN dich_vu dv 
    ...
    public function xoaLeTan($id)
    {
        $sql = "DELETE FROM le_tan WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([$id]);
    }

    public function getDanhSachTaiKham($trang_thai = null, $tu_ngay = null, $den_ngay = null)
    {
        $sql = "
            SELECT
                lk.id,
                ...
                lk.lich_kham_goc_id,
                lk.buoc_id,
                DATEDIFF(lk.ngay_kham, CURDATE()) AS con_lai,
                COALESCE(hs.ho_ten,  lk.ten_benh_nhan) AS ten_benh_nhan,
                COALESCE(hs.so_dien_thoai, lk.sdt)     AS sdt,
                hs.email,
                bs.ten_bac_si,
                dv.ten_dich_vu,
                bd.ten_buoc,
                bd.thu_tu AS buoc_thu_tu,
                (SELECT COUNT(*) FROM buoc_dich_vu WHERE dich_vu_id = lk.dich_vu_id) AS tong_buoc
            FROM lich_kham lk
            LEFT JOIN ho_so_benh_nhan hs ON hs.id = lk.ho_so_benh_nhan_id
            LEFT JOIN bac_si bs           ON bs.id = lk.bac_si_id
            LEFT JOIN dich_vu dv          ON dv.id = lk.dich_vu_id
            LEFT JOIN buoc_dich_vu bd     ON bd.id  = lk.buoc_id
            WHERE lk.la_tai_kham = 1
        ";

        $params = [];

        if ($trang_thai) {
            $sql .= " AND lk.trang_thai = :trang_thai";
            $params[':trang_thai'] = $trang_thai;
        }
        if ($tu_ngay) {
            $sql .= " AND lk.ngay_kham >= :tu_ngay";
            $params[':tu_ngay'] = $tu_ngay;
        }
        if ($den_ngay) {
            $sql .= " AND lk.ngay_kham <= :den_ngay";
            $params[':den_ngay'] = $den_ngay;
        }

        $sql .= " ORDER BY lk.ngay_kham ASC";
    ...
    public function capNhatTrangThaiTaiKham($id, $trang_thai)
    {
        $allowed = ['cho_kham', 'dang_kham', 'da_kham', 'vang_mat'];
        if (!in_array($trang_thai, $allowed)) return false;

        $stmt = $this->conn->prepare("UPDATE lich_kham SET trang_thai = ? WHERE id = ? AND la_tai_kham = 1");
        return $stmt->execute([$trang_thai, $id]);
    }

    public function taoLichTaiKham($lich_kham_id, $ngay_tai_kham, $ghi_chu = null, $buoc_id = null)
    {
        $sql = "SELECT ho_so_benh_nhan_id, bac_si_id, dich_vu_id, ten_benh_nhan, sdt
                FROM lich_kham WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([$lich_kham_id]);
        $goc = $stmt->fetch(PDO::FETCH_ASSOC);

        if (!$goc) return false;

        $check = $this->conn->prepare(
            "SELECT id FROM lich_kham WHERE lich_hen_id IS NULL AND la_tai_kham = 1
             AND ho_so_benh_nhan_id = ? AND trang_thai = 'cho_kham' AND ngay_kham = ?"
        );
        $check->execute([$goc['ho_so_benh_nhan_id'], $ngay_tai_kham]);
        if ($check->fetch()) return false;

        $sqlInsert = "INSERT INTO lich_kham
            (lich_hen_id, ho_so_benh_nhan_id, bac_si_id, dich_vu_id,
             ten_benh_nhan, sdt, ngay_kham, ghi_chu, trang_thai, la_tai_kham, lich_kham_goc_id, buoc_id, ngay_tao)
            VALUES (NULL, ?, ?, ?, ?, ?, ?, ?, 'cho_kham', 1, ?, ?, NOW())";

        $stmt2 = $this->conn->prepare($sqlInsert);
        return $stmt2->execute([
            $goc['ho_so_benh_nhan_id'],
            $goc['bac_si_id'],
            $goc['dich_vu_id'],
            $goc['ten_benh_nhan'],
            $goc['sdt'],
    ...
    public function getTaiKhamById($id)
    {
        $sql = "
            SELECT lk.id, lk.ngay_kham, lk.trang_thai, lk.ghi_chu, lk.lich_kham_goc_id,
                   COALESCE(hs.ho_ten, lk.ten_benh_nhan) AS ten_benh_nhan,
                   hs.email,
                   COALESCE(hs.so_dien_thoai, lk.sdt) AS sdt,
                   bs.ten_bac_si,
            ...
            LEFT JOIN bac_si bs           ON bs.id = lk.bac_si_id
            LEFT JOIN dich_vu dv          ON dv.id = lk.dich_vu_id
            WHERE lk.id = ? AND lk.la_tai_kham = 1
        ";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([$id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
...
ORDER BY lh.ngay_hen DESC, lh.gio_bat_dau DESC
    ...
    public function getLichKhamChiTietById($id)
    {
        $sql = "SELECT
                ...
                COALESCE(lk.dich_vu_id, dv.id) AS dich_vu_id,
                COALESCE(dv2.ten_dich_vu, dv.ten_dich_vu) AS ten_dich_vu,
            ...
            LEFT JOIN dich_vu dv2 ON lk.dich_vu_id = dv2.id
            LEFT JOIN buoc_dich_vu bd ON lk.buoc_id = bd.id
            WHERE lk.id = ?";
    ...
    public function getBuocByDichVuId($dich_vu_id)
    {
        $stmt = $this->conn->prepare(
            "SELECT * FROM buoc_dich_vu WHERE dich_vu_id = ? ORDER BY thu_tu ASC"
        );
        $stmt->execute([$dich_vu_id]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getBuocById($id)
    {
        $stmt = $this->conn->prepare("SELECT * FROM buoc_dich_vu WHERE id = ?");
        $stmt->execute([$id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }

    public function addBuocDichVu($dich_vu_id, $thu_tu, $ten_buoc, $mo_ta)
    {
        $stmt = $this->conn->prepare(
            "INSERT INTO buoc_dich_vu (dich_vu_id, thu_tu, ten_buoc, mo_ta) VALUES (?, ?, ?, ?)"
        );
        return $stmt->execute([$dich_vu_id, $thu_tu, $ten_buoc, $mo_ta ?: null]);
    }

    public function updateBuocDichVu($id, $thu_tu, $ten_buoc, $mo_ta)
    {
        $stmt = $this->conn->prepare(
            "UPDATE buoc_dich_vu SET thu_tu = ?, ten_buoc = ?, mo_ta = ? WHERE id = ?"
        );
        return $stmt->execute([$thu_tu, $ten_buoc, $mo_ta ?: null, $id]);
    }

    public function deleteBuocDichVu($id)
    {
        $stmt = $this->conn->prepare("DELETE FROM buoc_dich_vu WHERE id = ?");
        return $stmt->execute([$id]);
    }

    public function getBuocTiepTheo($buoc_id, $dich_vu_id)
    {
        $current = $this->getBuocById($buoc_id);
        if (!$current) return null;
        $stmt = $this->conn->prepare(
            "SELECT * FROM buoc_dich_vu WHERE dich_vu_id = ? AND thu_tu > ? ORDER BY thu_tu ASC LIMIT 1"
        );
        $stmt->execute([$dich_vu_id, $current['thu_tu']]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    }
    ...
    {
        $stmt = $this->conn->query("SELECT * FROM vat_tu WHERE danh_muc = 'tieu hao'");
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }
    ...
    // ========== Mạnh đẹp zai ==========
    public function saveResultEx($lich_kham_id, $danh_sach_dv, $chan_doan, $don_thuoc, $huong_dieu_tri = null)
    {
        try {
            $this->conn->beginTransaction();
            ...
            $stmt->execute([
                ':chan_doan'       => $chan_doan,
                ':huong_dieu_tri'  => $huong_dieu_tri ?: null,
            ...
            if (!empty($danh_sach_dv)) {
                ...
                $sqlInsertDV = "INSERT INTO ct_ls_kham
                            (ls_kham_id, ten_dich_vu, dich_vu_id, don_gia)
                            VALUES
                            (:ls_kham_id, :ten_dich_vu, :dich_vu_id, :don_gia)";
                ...
                foreach ($danh_sach_dv as $dv) {
                    $gia = $this->getGiaDichVu($dv['dich_vu_id']);
...
GROUP BY lsk.id;";
    ...
    public function insertXray($data)
    {
        $sql = "INSERT INTO xray
            (ho_so_benh_nhan_id, lich_kham_id, lich_su_kham_id, bac_si_id,
             loai_xray, vi_tri, file_path, mo_ta, ai_phan_tich, ngay_chup)
            VALUES (?,?,?,?,?,?,?,?,?,?)";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([
            $data['ho_so_benh_nhan_id'],
            $data['lich_kham_id']    ?? null,
            $data['lich_su_kham_id'] ?? null,
            $data['bac_si_id']       ?? null,
            $data['loai_xray'],
            $data['vi_tri']          ?? null,
            $data['file_path'],
            $data['mo_ta']           ?? null,
            $data['ai_phan_tich']    ?? null,
            $data['ngay_chup'],
        ]);
    }

    public function lastXrayId()
    {
        return $this->conn->lastInsertId();
    }

    public function getXrayById($id)
    {
        $sql = "SELECT x.*, bs.ten_bac_si, hs.ho_ten
                FROM xray x
                LEFT JOIN bac_si bs ON x.bac_si_id = bs.id
                LEFT JOIN ho_so_benh_nhan hs ON x.ho_so_benh_nhan_id = hs.id
                WHERE x.id = ?";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([$id]);
        return $stmt->fetch(PDO::FETCH_ASSOC);
    ...
    public function getXrayByLichKham($lich_kham_id)
    {
        $sql = "SELECT x.*, bs.ten_bac_si
                FROM xray x
                LEFT JOIN bac_si bs ON x.bac_si_id = bs.id
                WHERE x.lich_kham_id = ?
    ...
    public function getXrayByPatient($ho_so_id)
    {
        $sql = "SELECT x.*, bs.ten_bac_si
                FROM xray x
                LEFT JOIN bac_si bs ON x.bac_si_id = bs.id
                WHERE x.ho_so_benh_nhan_id = ?
                ORDER BY x.ngay_chup DESC";
        $stmt = $this->conn->prepare($sql);
        $stmt->execute([$ho_so_id]);
        return $stmt->fetchAll(PDO::FETCH_ASSOC);
    }

    public function getAllXray($bac_si_id = null)
    {
        $sql = "SELECT x.*, bs.ten_bac_si, hs.ho_ten
                FROM xray x
                LEFT JOIN bac_si bs ON x.bac_si_id = bs.id
    ...
    public function updateXrayAi($id, $ai_text)
    {
        $sql = "UPDATE xray SET ai_phan_tich = ? WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([$ai_text, $id]);
    }

    public function updateXrayKetQua($id, $ket_qua)
    {
        $sql = "UPDATE xray SET ket_qua_bac_si = ?, trang_thai = 'da_doc' WHERE id = ?";
        $stmt = $this->conn->prepare($sql);
        return $stmt->execute([$ket_qua, $id]);
    }

    public function deleteXray($id)
    {
        $stmt = $this->conn->prepare("DELETE FROM xray WHERE id = ?");
        return $stmt->execute([$id]);
    }

    public function countXrayByPatient($ho_so_id)
    {
        $stmt = $this->conn->prepare("SELECT COUNT(*) FROM xray WHERE ho_so_benh_nhan_id = ?");
        $stmt->execute([$ho_so_id]);
        return $stmt->fetchColumn();
    }
}
