...
class SuppliesController
{
    ...
    public function addMaterials()
    {
        if ($_SERVER['REQUEST_METHOD'] == 'POST') {
            ...
            $type = in_array($type, [0, 1], true) ? $type : 0;
            $trang_thai_su_dung = isset($_POST['trang_thai_su_dung']) ? (int) $_POST['trang_thai_su_dung'] : 1;
            $trang_thai_su_dung = in_array($trang_thai_su_dung, [0, 1], true) ? $trang_thai_su_dung : 1;
    ...
    public function toggleMaterialProductStatus()
    {
        ...
        $item = $this->clinic->findMaterials($id);
        if (!$item || (int) ($item['type'] ?? 0) !== 1) {
            $_SESSION['error'] = 'Chỉ có thể khóa hoặc mở sản phẩm.';
            header("Location: admin.php?admin=materials");
            exit;
        }

        $status = $action === 'khoa' ? 0 : 1;
        $ok = $this->clinic->updateProductSaleStatus($id, $status);

        $_SESSION[$ok ? 'success' : 'error'] = $ok
            ? ($status ? 'Mở bán sản phẩm thành công.' : 'Khóa sản phẩm thành công.')
            : 'Thao tác thất bại.';
