...
class AppointmentController
{
    ...
    public function suaYeuCauDatLich()
    {
        ...
        if ($ngay === '' || strtotime($ngay) < strtotime(date('Y-m-d'))) {
            $_SESSION['error'] = "Ngày khám không hợp lệ hoặc đã qua.";
            header("Location: admin.php?admin=formSuaYeuCauDatLich&id=$id"); exit;
        }
        if ($trangThai === 'da_xac_nhan' && empty($bacSiId)) {
            $_SESSION['error'] = "Phải chọn bác sĩ trước khi xác nhận lịch.";
            header("Location: admin.php?admin=formSuaYeuCauDatLich&id=$id"); exit;
        }
        if ($trangThai === 'da_xac_nhan' && empty($gioBatDau)) {
